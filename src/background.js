...
ipcMain.handle("open:file", async (event, options = {filters: [{ name: 'All Files', extensions: ['*'] }]}) => {
  const result = {
    error: false,
    errorMessage: "",
    data: {
  ...
  let filePaths = dialog.showOpenDialogSync(win, dialogOptions);
